...
const InputQuantity = ({ limit }) => {
  ...
  return (
    <form className="flex justify-items-stretch">
      <span className={spanClassName} onClick={() => dispatch(decrement())}>
        <FiMinus />
      </span>
      <input
        ...
        value={quantity}
        onChange={(e) =>
          dispatch(setQuantity({ value: e.target.value, limit }))
        }
      />
      <span className={spanClassName} onClick={() => dispatch(increase())}>
        <BsPlusLg />
      </span>
    </form>
  );
